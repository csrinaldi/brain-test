...
- [ ] Session memory captured as a record (`brain:memory:save --issue N`); it reaches `main`
      ...
      record does NOT satisfy it — though a record already on `main` from its own lane PR
      does, with no rebase needed. `skip:memory-gate` is honored at the "standard" tier
      when applied by someone other than the PR author; "regulated" refuses it; "lite"
      does not consult it.
